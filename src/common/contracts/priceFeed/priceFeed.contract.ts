...
function getPriceFeedContract(feedAddress: string): PriceFeed & Contract {
    return new Contract(feedAddress, abiJson, new JsonRpcProvider(`https://mainnet.infura.io/v3/df99674aba224b7194774125b0712c95`)) as PriceFeed & Contract
}
...
export async function getTokenPriceInUSDCent(symbol: string) {
    ...
    if (feedAddress === undefined) throw new NotFoundException("symbol feed not supported")
    return Number(BigInt((await getPriceFeedContract(feedAddress.address).latestRoundData()).answer) / BigInt(10**6))
}
